/**
 * Version Management Routes - CV-007
 *
 * API endpoints for version comparison, diff viewing, and export functionality
 */
...
// Validation schemas
const compareVersionsSchema = Joi.object({
  version_a_id: Joi.number().integer().positive().required(),
  version_b_id: Joi.number().integer().positive().required(),
  diff_type: Joi.string().valid('text', 'structural', 'metadata', 'all').default('all'),
  include_unchanged: Joi.boolean().default(false),
  algorithm: Joi.string().valid('myers', 'patience', 'histogram', 'semantic').default('myers'),
  granularity: Joi.string().valid('line', 'word', 'character').default('line'),
  context_lines: Joi.number().integer().min(0).max(10).default(3),
  ignore_whitespace: Joi.boolean().default(false)
});

const exportDiffSchema = Joi.object({
  version_ids: Joi.array().items(Joi.number().integer().positive()).min(2).max(2).required(),
  format: Joi.string().valid('pdf', 'html', 'json', 'docx').required(),
  include_metadata: Joi.boolean().default(true),
  include_statistics: Joi.boolean().default(true),
  include_unchanged: Joi.boolean().default(false),
  template: Joi.string().optional(),
  custom_branding: Joi.boolean().default(false),
  page_orientation: Joi.string().valid('portrait', 'landscape').default('portrait')
});

const getDiffSchema = Joi.object({
  format: Joi.string().valid('json', 'html', 'unified').default('json'),
  context_lines: Joi.number().integer().min(0).max(10).default(3)
});

const changesSummarySchema = Joi.object({
  compare_with: Joi.string().valid('previous', 'published', 'specific').default('previous'),
  compare_version_id: Joi.number().integer().positive().optional()
});

export function createVersionRoutes(pool: Pool): Router {
  const versionService = new VersionService(pool);
  const diffService = new DiffService(pool);

  /**
   * GET /api/versions/compare
   * Compare two versions with full diff analysis
   */
   ...
   * Get pre-computed diff between two specific versions
   */
  router.get(
    '/:id1/diff/:id2',
    authenticate,
    validateQuery(getDiffSchema),
    async (req: Request, res: Response) => {
      try {
  ...
  router.post(
    '/diff/export',
    authenticate,
    validateRequest(exportDiffSchema),
    async (req: Request, res: Response) => {
      try {
        const {
          version_ids,
        ...
        const exportResult = await diffService.exportDiff(
          diffResult.data!,
          format,
          {
  ...
  /**
   * GET /api/versions/:id/changes-summary
   * Get change summary for a version compared to previous
   */
  router.get(
    '/:id/changes-summary',
    authenticate,
    validateQuery(changesSummarySchema),
    async (req: Request, res: Response) => {
      try {
        const { id } = req.params;
        ...
        if (compare_with === 'specific' && compare_version_id) {
          ...
          const publishedResult = await pool.query(
            `SELECT id FROM content_versions
             ...
             LIMIT 1`,
            [targetVersion.site_id, targetVersion.content_type, targetVersion.content_id]
          );
  ...
  /**
   * GET /api/versions/history/:contentType/:contentId
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const versionsResult = await pool.query(
          `SELECT v.*, u.email as author_email, u.name as author_name
        ...
        // Validate user access to the site
 ...
 * Generate unified diff format
 */
function generateUnifiedDiff(diff: any): string {
  const lines: string[] = [];
  ...
  diff.textDiff.hunks.forEach((hunk: any) => {

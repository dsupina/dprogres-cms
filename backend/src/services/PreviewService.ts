...
import { BlockNode, BlockRenderResult, RenderedBlock } from '../types/content';
import { renderBlocksToHtml, renderBlock, getMissingFields } from '../utils/blockRendering';
import { collectMissingBlockFields, normaliseBlock } from '../utils/contentBlocks';
...
export class PreviewService extends EventEmitter {
  ...
  async assembleBlockPreview(
    ...
    const preparedBlocks = blocks.map((block) => normaliseBlock(block));
    ...
    const html = DOMPurify.sanitize(renderBlocksToHtml(hydratedBlocks));
    const tree = this.renderBlockTree(hydratedBlocks);
    const missingFields = collectMissingBlockFields(hydratedBlocks);
  ...
  private async hydrateBlocksWithAI(
    blocks: BlockNode[],
    options: { topic?: string },
    stats: { aiCalls: number; completedBlocks: number }
  ): Promise<BlockNode[]> {
    ...
    for (const block of blocks) {
      ...
      const children = normalised.children && normalised.children.length > 0
        ...
        : [];
      const candidate: BlockNode = { ...normalised, children };
  ...
  private renderBlockTree(blocks: BlockNode[]): RenderedBlock[] {
  ...
  private async requestBlockCompletion(
    block: BlockNode,
    missingFields: string[],
    context: { topic?: string }
  ): Promise<{ props: Record<string, any>; summary?: string; provider?: string; confidence?: number } | null> {
    ...
    const fetchFn: typeof globalThis.fetch | undefined = (globalThis as any).fetch;
  ...
  private buildFallbackSuggestion(
    ...
    context: { topic?: string }
  ): { props: Record<string, any>; summary?: string; provider?: string; confidence?: number } | null {
